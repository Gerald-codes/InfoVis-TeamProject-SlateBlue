...
The redesign replaces the stacked area chart with a compact sparkline summary table, allowing each country’s historical trend to be viewed independently without comparing stacked layers. It focuses on the five highest-emitting countries in 2023 and removes broad aggregated categories such as the European Union and Rest of World. The table also reports total emissions, per-capita emissions, percentage share of global emissions and recent five-year change in a single view. Distinct colourblind-friendly colours make the countries easier to distinguish.
...
- **Integrated summary metrics** showing total emissions, per-capita emissions, percentage share, and five-year change alongside each sparkline
...
### 4.3 Data Import and Cleaning Pipeline
...
#### 4.3.3 Identify the Five Highest-Emitting Countries
...
#### 4.3.4 Clean and Convert Emissions Data
...
#### 4.3.5 Save Cleaned Emissions Data
...
#### 4.3.7 Clean and Merge Population Data
...
#### 4.3.8 Compute Per-Capita Emissions
...
# Reshape and clean the GDP dataset
gdp_clean <- gdp_raw |>
  
  filter(`Indicator Code` == "NY.GDP.MKTP.CD") |>

  # Keep only the countries used in the sparkline table
  filter(`Country Name` %in% country_lookup$wb_name) |>

  # Keep the country identifier and year columns
  select(
    `Country Name`,
    matches("^\\d{4}$")
  ) |>

  # Convert the wide year columns into tidy country-year rows
  pivot_longer(
    cols = -`Country Name`,
    names_to = "year",
    values_to = "gdp_usd",
  ...
  filter(
    between(year, GDP_START_YEAR, GDP_END_YEAR)
  ) |>

  # Match World Bank country names to the project country names
  left_join(
    country_lookup |>
...
### Calculate 5 year GDP percentage change
```{r}
...
write_csv(
  gdp_clean,
  "data/cleaned/gdp_2018_2023.csv"
)

write_csv(
  gdp_change_5yr,
  "data/cleaned/gdp_change_2018_2023.csv"
)
```

#### 4.3.9 Quick Validation
...
4. **The table supports quick comparison.** Historical trends, latest totals, per-capita values, global shares and five-year changes are aligned in one compact display. Distinct Okabe–Ito colours help readers separate the five countries.
...
save_sparkline <- function(data, country_name, colour) {
...
spark_data |>
  ...
  cols_label(
    country = "Country",
    spark_path = "Trend (1960–2023)",
    latest = "GtCO₂ (2023)",
    per_capita = "tCO₂/person (2023)",
    share = "% Share (2023)",
    change_5yr = "5yr Change (%) (2019-2023)"
  ) |>
  tab_header(
    title = md(
      "**Top Five Fossil CO₂ Emitting Countries**"
    ),
    subtitle = md(
      "Historical trends and 2023 key indicators (1960–2023)"
    )
  ) |>
  fmt_number(
    columns = latest,
