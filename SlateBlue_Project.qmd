...
This redesign is intended for **general readers of climate news** who want to compare how the largest-emitting countries contribute to global fossil CO₂ emissions. It supports comparisons of **national totals** and **per-capita emissions**, showing contributions and population-adjusted differences. The five highest emitters in 2023 are shown because together they produced about 60% of global fossil CO₂ emissions. This keeps the comparison compact without hiding differences inside aggregated regional categories.

**Main revisions:**

- Focus on the five highest-emitting countries for direct comparison
- Use a sparkline summary table to separate historical trends
- Pair trends with totals, per-capita values, global share, and emissions and nominal GDP changes from 2019 to 2023
- Apply distinct Okabe–Ito colours with direct country labels
- Encode increases and decreases with arrows, signs, and blue/orange styling
...
as **nominal GDP change**. It provides current-price economic context only and
should not be interpreted as real GDP growth or evidence that GDP changes caused
changes in fossil CO₂ emissions.
...
gdp_change_2019_2023 <- gdp_clean |>
  ...
  mutate(
    gdp_pct_change_2019_2023 = (gdp_end / gdp_start - 1) * 100
  ) |>
...
4. **The table supports quick comparison.** Historical trends, latest totals, per-capita values, global shares, emissions changes and nominal GDP changes from 2019 to 2023 are aligned in one compact display. Distinct Okabe–Ito colours help readers separate the five countries.
...
Nominal GDP change is included as current-price economic context for the same period. It is not a measure of real GDP growth and does not establish that GDP changes caused changes in fossil CO₂ emissions.
...
# Accessible signed-change colours with at least 4.5:1 contrast on white
delta_colours <- c(
  "increase" = "#B34700",
  "decrease" = "#0072B2"
)
...
spark_data <- emissions_percapita |>
  ...
  mutate(
    emissions_pct_change_2019_2023 = (latest / emissions_2019 - 1) * 100,
...
spark_data <- spark_data |>
  mutate(
    gdp_change_2019_2023_display = case_when(
      ...
      gdp_pct_change_2019_2023 < 0 ~ paste0(
        "▼ −",
        sprintf("%.1f%%", abs(gdp_pct_change_2019_2023))
      ),
      TRUE ~ sprintf(
        "%.1f%%",
        gdp_pct_change_2019_2023
    ...
    emissions_change_2019_2023_display = case_when(
      ...
      emissions_pct_change_2019_2023 < 0 ~ paste0(
        "▼ −",
        sprintf("%.1f%%", abs(emissions_pct_change_2019_2023))
      ),
...
spark_data |>
  ...
  cols_label(
    ...
    share = "Share (2023)",
    gdp_change_2019_2023_display = html(
      "Nominal GDP Δ<br>(2019–2023)"
    ),
    emissions_change_2019_2023_display = html(
      "CO₂ Δ<br>(2019–2023)"
    )
  ) |>
  tab_header(
  ...
  tab_source_note(md(
    "**Interpretation:** GDP change is nominal and provides current-price context only; it is not real GDP growth or evidence of causation."
  )) |>
  tab_source_note(md(
    "**Change encoding:** Orange ▲ indicates an increase and blue ▼ indicates a decrease. Colour communicates direction, not desirability."
  )) |>
  tab_source_note(md(
  ...
  cols_width(
    ...
    per_capita ~ px(80),
    share ~ px(65),
    gdp_change_2019_2023_display ~ px(110),
    emissions_change_2019_2023_display ~ px(95)
  ) |>
...
- [AI Interaction Log](ai_logs/ai_logs_merged.md)
